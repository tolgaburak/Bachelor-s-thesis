...
# test model
model = DecisionTreeRegressor(max_depth=3, min_samples_split=2, criterion="squared_error").fit(X_train, y_train)
...
final_model = DecisionTreeRegressor(max_depth=3, min_samples_split=2, criterion="squared_error").fit(X_train, y_train)
forecast = final_model.predict(X_test)

# forecast result (forecasted after 2024-03-01 to 2024-06-30, mart,nisan,mayıs,haziran)
...
ax.set_xlabel('Tarih')
ax.set_title('Forecasted Result')
plt.show()
